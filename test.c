...
extern void l3_init(crc_t* crc_table);
extern crc_t calc_crc(char *buffer, int length, crc_t* crc_table);
extern bool check_crc(char *buffer, int length, crc_t* crc_table);
...
void test_calc_crc() {
    ...
    char *buf = "123456789";
    assert((crc_t) calc_crc(buf, strlen(buf), crc_table) == 0x89A1897F);
    buf = "fsdjkfsdafdsdsf";
    assert((crc_t) calc_crc(buf, strlen(buf), crc_table) == 0xC664EABF);
    buf = "ruerweojdcdnsci212332";
    assert((crc_t) calc_crc(buf, strlen(buf), crc_table) == 0xF27C4D33);
...
void test_check_crc() {
    ...
    assert(check_crc(buf, 13, crc_table));
    buf[4] = 6;
    assert(check_crc(buf, 13, crc_table) == false); 
    buf[11] = 0x88;
    assert(check_crc(buf, 13, crc_table) == false);

    char buf2[12] = {'a', 'b', 'c', 'd', '6', '4', '5', '7', 0x59, 0x82, 0x68, 0x0B};
    assert(check_crc(buf2, 12, crc_table));
